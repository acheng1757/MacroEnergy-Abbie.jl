...
function add_model_constraint!(ct::AgeBasedRetirementConstraint, y::Union{AbstractEdge,AbstractStorage}, model::Model)
    ...
    if ret_period==0
        ...
        ct.constraint_ref = @constraint(
            model, 
            min_retired_capacity(y) <= sum(retired_capacity_track(y,k) for k=1:curr_period) + sum(retrofitted_capacity_track(y,k) for k=1:curr_period)
        )
    else
        #### All new capacity built up to the retirement period must either retire or be retrofitted in the current period
        ct.constraint_ref = @constraint(
            model, 
            sum(new_capacity_track(y,k) for k=1:ret_period) + min_retired_capacity(y) <= sum(retired_capacity_track(y,k) for k=1:curr_period) + sum(retrofitted_capacity_track(y,k) for k=1:curr_period)
        )
