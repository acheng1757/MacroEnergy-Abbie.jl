...
function default_data(t::Type{DrEaf}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{DrEaf}, id=missing)
...
function simple_default_data(::Type{DrEaf}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_expand => true,
...
function set_commodity!(::Type{DrEaf}, commodity::Type{<:Commodity}, data::AbstractDict{Symbol,Any})
...
function make(asset_type::Type{DrEaf}, data::AbstractDict{Symbol,Any}, system::System)
    id = AssetId(data[:id])
