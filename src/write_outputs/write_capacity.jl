...
function get_optimal_capacity_by_field(
    ...
    # Only keep (obj, field) pairs where the field actually applies to the object
    ...
    if isempty(obj_asset_map)
        return DataFrame(
            ...
            resource_id = [get_component_id(obj) for (obj, f) in valid_pairs],  # component id is same as resource id
